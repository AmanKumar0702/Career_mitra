...
import {
  ...
  Briefcase, Palette, Heart, SlidersHorizontal, X, Layers,
} from "lucide-react";
import { categories, educationGroups } from "@/data/courses";
...
function CourseCard({ course, i, enrolled, enrolling, onEnroll, onNavigate }: {
  course: any; i: number; enrolled: boolean; enrolling: string | null;
  ...
  return (
    <motion.div
      ...
      onClick={onNavigate}
      className="group cursor-pointer bg-white dark:bg-[#0f172a] rounded-2xl border border-gray-100 dark:border-[#1e293b] overflow-hidden flex flex-col"
    ...
    >
      <div className={`relative h-36 bg-gradient-to-br ${cfg.gradient} flex items-center justify-center overflow-hidden`}>
        ...
        <motion.div whileHover={{ scale: 1.1, rotate: 5 }} transition={{ duration: 0.3 }} className="relative z-10 w-14 h-14 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center">
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center gap-2 mb-3">
          {lvl && <span className="text-xs font-semibold px-2 py-0.5 rounded-md" style={{ background: lvl.bg, color: lvl.color }}>{lvl.label}</span>}
          <span className="text-xs font-medium text-gray-400 dark:text-slate-500">{course.category}</span>
          {course.featured && <span className="ml-auto text-xs font-bold text-amber-500 bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded-md">Featured</span>}
        </div>
        <h3 className="font-bold text-gray-900 dark:text-white mb-1.5 line-clamp-2 text-sm leading-snug group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{course.title}</h3>
        <p className="text-xs text-gray-600 dark:text-slate-400 line-clamp-2 mb-4 flex-1 leading-relaxed">{course.description}</p>
        {course.instructor && (
        ...
        <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-slate-500 mb-4 pt-3 border-t border-gray-50 dark:border-[#1e293b]">
          ...
          <span className="flex items-center gap-1 ml-auto"><Clock className="w-3.5 h-3.5" />{course.duration}</span>
        </div>
        <motion.button
          ...
          className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 ${
            enrolled ? "bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800" : "btn-primary"
          }`}
        >
          {enrolling === course._id ? <Loader2 className="w-4 h-4 animate-spin" /> : enrolled ? <><CheckCircle className="w-4 h-4" /> Continue Learning</> : <><Play className="w-4 h-4" /> Enroll Free</>}
        </motion.button>
      </div>
    </motion.div>
  );
...
export default function LearnPage() {
  ...
  const filtered = courses.filter((c) => {
    ...
    const matchGroup = activeGroup === "all" || c.educationGroup === activeGroup;
    return matchSearch && matchLevel && matchGroup;
  });

  // Group courses by educationGroup for structured view
  const groupedCourses = educationGroups.map((g) => ({
    ...g,
    courses: filtered.filter((c) => c.educationGroup === g.id),
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0f1e]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-slate-500 mb-3 flex items-center gap-1.5">
            <Layers className="w-3.5 h-3.5" /> Education Level
          </p>
          <div className="flex gap-2 flex-wrap">
            <motion.button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all border ${
                activeGroup === "all"
                  ...
                  : "bg-white dark:bg-[#0f172a] text-gray-600 dark:text-slate-400 border-gray-200 dark:border-[#1e293b] hover:border-blue-400"
              }`}
            >
              All Levels
            </motion.button>
              ...
              >
                ...
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${group.color} flex items-center justify-center flex-shrink-0`}>
                      <GraduationCap className="w-5 h-5 text-white" />
                    </div>
                    <div>
                      <h2 className="font-bold text-gray-900 dark:text-white text-lg leading-tight">{group.label}</h2>
                      <p className="text-xs text-gray-400 dark:text-slate-500">{group.desc} · {group.courses.length} course{group.courses.length !== 1 ? "s" : ""}</p>
                    </div>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {group.courses.slice(0, 3).map((course, i) => (
                    <CourseCard key={course._id} course={course} i={i} enrolled={isEnrolled(course._id)} enrolling={enrolling} onEnroll={handleEnroll} onNavigate={() => router.push(`/learn/${course._id}`)} />
                  ))}
                </div>
              </motion.div>
            ))}
